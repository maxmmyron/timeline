...
export const createNode = <T extends (...args: any) => any>(name: string, transform: T, initialIn: Parameters<T>[0] extends undefined ? null : Parameters<T>[0], initialOut: ReturnType<T> extends void ? null : ReturnType<T>): App.EditorNode<T> =>{
  return ({
    uuid: uuidv4(),
    name,
    transform,
